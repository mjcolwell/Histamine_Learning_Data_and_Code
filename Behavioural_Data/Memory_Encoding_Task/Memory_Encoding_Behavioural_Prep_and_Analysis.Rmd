...
# Win trials - LR ~ unseen distractor v

med.fit <- lm(LR_recipmodel.x ~ `v(0)`, data = Merged_DDM_Meta)

summary(med.fit)

# Win trials - inv. decision temp ~ unseen distractor v

med.fit <- lm(inv_recipmodel.x ~ `v(0)`, data = Merged_DDM_Meta)

summary(med.fit)

# Win trials - LR ~ previously encoded v

med.fit <- lm(LR_recipmodel.x ~ `v(1)`, data = Merged_DDM_Meta)

summary(med.fit)

# Win trials - inv. decision temp ~ previously encoded v

med.fit <- lm(inv_recipmodel.x ~ `v(1)`, data = Merged_DDM_Meta)

summary(med.fit)

####

# Loss trials - LR ~ unseen distractor v

med.fit <- lm(LR_recipmodel.y ~ `v(0)`, data = Merged_DDM_Meta)

summary(med.fit)

# Loss trials - inv. decision temp ~ unseen distractor v

med.fit <- lm(inv_recipmodel.y ~ `v(0)`, data = Merged_DDM_Meta)

summary(med.fit)

# Loss trials - LR ~ previously encoded v

med.fit <- lm(LR_recipmodel.y ~ `v(1)`, data = Merged_DDM_Meta)

summary(med.fit)

# Loss trials - inv. decision temp ~ previously encoded v

med.fit <- lm(inv_recipmodel.y ~ `v(1)`, data = Merged_DDM_Meta)

summary(med.fit)

# Only the working memory v ~ RL params analysis is reported in the Supplementary Mats., as v during working memory is a more representative measure of general cognitive ability. All results remain non-significant.
