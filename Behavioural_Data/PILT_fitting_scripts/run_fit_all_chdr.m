...
for sub=1:size(subjects,2)
    ...
    for vis = 1:size(visitnum, 2)
        ...
        if ~isfield(data,'missing')
            ...
            % dont fit the first 10 trials of each run
           % run_starts = find(fitdata.newrun==1);                                 % first trial of each run
           % run_ends   = [run_starts(2:end)-1; numel(fitdata.newrun)];            % last trial of each run
           % for k = 1:numel(run_starts)
           %     drop_idx = run_starts(k):min(run_starts(k)+9, run_ends(k));       % first 10 (or to run end)
           %     fitdata.nochoice(drop_idx) = 1;                                   % exclude from likelihood
           % end
